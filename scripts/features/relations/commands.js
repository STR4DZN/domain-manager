...
export async function executeRelationUpsert({ payload, callerUserId }) {
  ...
  const domain=resolve(n.domain,RECORD_TYPES.DOMAIN); const target=resolve(n.target,RECORD_TYPES.DOMAIN); assertDiplomacy(domain);
...
export async function executeRelationRemove({ payload, callerUserId }) {
  assertGM(callerUserId); const n=normalizeRelationRemovePayload(payload); const domain=resolve(n.domain,RECORD_TYPES.DOMAIN); assertDiplomacy(domain);
...
export async function executeAgreementUpdate({ payload, callerUserId }) {
  assertGM(callerUserId); const n=normalizeAgreementUpdatePayload(payload); const agreement=resolve(n.agreement,RECORD_TYPES.AGREEMENT); const parties=n.parties.map((p)=>resolve(p,RECORD_TYPES.DOMAIN)); parties.forEach(assertDiplomacy); const refs=parties.map(ref); validateTransfers(n.transfers,refs);
...
export async function executeAgreementStatus({ payload, callerUserId }) {
  assertGM(callerUserId); const n=normalizeAgreementStatusPayload(payload); const agreement=resolve(n.agreement,RECORD_TYPES.AGREEMENT); const before=foundry.utils.deepClone(agreement.data); const data={...foundry.utils.deepClone(agreement.data),status:n.status};
