...
function revision(value){ if(value==null||value==="")return null; const n=Number(value); if(!Number.isSafeInteger(n)||n<0)throw new ModuleError(ERROR_CODES.VALIDATION,"expectedModifiedTime precisa ser inteiro não-negativo."); return n; }
...
export function normalizeIntelUpsertPayload(payload={}){
  ...
  return { domain:domainRef(payload.domain), expectedModifiedTime:revision(payload.expectedModifiedTime), localId:clean(payload.localId), title, category, visibility, targetDomain:optionalDomainRef(payload.targetDomain), content:clean(payload.content), credibility, source:clean(payload.source), revealed:visibility==="public"||payload.revealed===true, tags:tags(payload.tags) };
}
export function normalizeIntelRemovePayload(payload={}){ const localId=clean(payload.localId); if(!localId) throw new ModuleError(ERROR_CODES.VALIDATION,"localId de intel é obrigatório."); return {domain:domainRef(payload.domain),expectedModifiedTime:revision(payload.expectedModifiedTime),localId}; }
